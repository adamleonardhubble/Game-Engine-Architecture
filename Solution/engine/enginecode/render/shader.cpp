...
{
	Shader * Shader::Create(const std::string& filepath) // Create, compile and link a shader from a single filepath source
	{
		...
		{
			return new OpenGLShader(filepath); // Return a new instance of an OpenGL API shader programme
			break; // Break from the switch case statement
		}
	...
	Shader * Shader::Create(const std::string& vertexFilepath, const std::string& fragmentFilepath) // Create, compile and link an OpenGL API shader from multiple filepath sources
	{
		switch (RenderAPI::getAPI()) // Switch case statement, return the current renderer API
		{
		case RenderAPI::API::None: // If there is renderer API currently set, do the following
		...
		case RenderAPI::API::OpenGL: // If the renderer API currently set is 'OpenGL', do the following
		{
			return new OpenGLShader(vertexFilepath, fragmentFilepath); // Return a new instance of an OpenGL API shader programme
			break; // Break from the switch case statement
		}
		case RenderAPI::API::Direct3D: // If the renderer API currently set is 'Direct3D', do the following
		{

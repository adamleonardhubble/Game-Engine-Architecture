...
{
	void OpenGLTextRenderer::actionCommand(RenderCommand * command) // Invoke a render command
	{
	...
	void OpenGLTextRenderer::submit(const std::shared_ptr<Material>& material) // Submit a material instance for drawing
	{
		...
		glDrawElements(GL_QUADS, Geometry->getCount(), GL_UNSIGNED_INT, nullptr); // Draw the geometry (issue a draw call for the preceeding material)
	}
}